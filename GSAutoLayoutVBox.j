...
@implementation GSAutoLayoutVBoxViewInfo: CPObject
{
  var _view;
...
- (GSAutoLayoutVBoxViewInfo) infoForView: (CPView)aView
{
  ...
  if (_displayAutoLayoutContainers)
    {
      ...
      {
	var i, count = [_vManager linePartCount];
	...
	for (i = 0; i < count; i++)
	  {
		...
		[path moveToPoint: CPMakePoint (CPMinX (bounds), s.position)];
		[path lineToPoint: CPMakePoint (CPMaxX (bounds), s.position)];
		[path stroke];
	      }
	  }
      }
